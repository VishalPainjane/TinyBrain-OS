...
Implementation complete in `internal/inference/llama/` — pending Linux CUDA compile verify + manual GPU checklist + matrix update on merge
...
- [ ] `inference-cgo` CPU job passes (unchanged) — verify on PR
- [ ] `go build -tags cuda ./internal/inference/llama/...` succeeds on Linux with CUDA toolkit + GGML_CUDA build
...
- [ ] Matrix CUDA rows updated to **Partial** or **Yes** per verification level (on merge)
